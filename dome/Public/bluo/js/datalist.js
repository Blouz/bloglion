layui.define(['laypage', 'layer', 'form', 'pagesize'], function (exports) {
    ...
    function initilData(currentIndex, pageSize) {
        ...
        var data = new Array();
        $.ajax({
              url:'http://blog.com/admin.php/Login/adm_show.html',
              type:"get",
        ...
        setTimeout(function () {
            ...
            html += '<thead><tr><th>账号</th><th>密码</th><th>创建时间</th><th>最后登录时间</th><th>ip</th><th colspan="2">选项</th><th colspan="2">操作</th></tr></thead>';
            html += '<tbody>';
            //遍历文章集合
            for (var i = 0; i < data.length; i++) {
                var item = data[i];
                html += "<tr>";
                html += "<td><span class='upda'>" + item.adm_name + "</span><input type='hidden' class='hide' idd="+ item.id +" ></td>";
                html += "<td><a href='http://blog.com/admin.php/Login/adm_update.html?id="+item.id+"'>" + "*****" + "</a></td>";
                html += "<td>" + item.adm_creat_time + "</td>";
                html += "<td>" + item.adm_last_time + "</td>";
                html += "<td>" + item.adm_ip + "</td>";
                ...
                html += '<td><button class="layui-btn layui-btn-small layui-btn-normal adm_del" idd="' + item.id + '"><i class="layui-icon">&#xe642;</i></button></td>';
                html += '<td><button class="layui-btn layui-btn-small layui-btn-danger del"  idd="' + item.id + '"><i class="layui-icon">&#xe640;</i></button></td>';
                html += "</tr>";
            }
            html += '</tbody>';
            html += '</table>';
