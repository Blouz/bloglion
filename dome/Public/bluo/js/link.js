...
layui.define(['laypage', 'layer', 'form', 'pagesize'], function (exports) {
    ...
    function initilData(currentIndex, pageSize) {
        ...
        var data = new Array();
        $.ajax({
              url:'http://blog.com/admin.php/Link/link_Show.html',
              type:"get",
        ...
        setTimeout(function () {
            ...
            html += '<thead><tr><th>ID</th><th>名称</th><th>网站链接</th><th colspan="2">操作</th></tr></thead>';
            html += '<tbody>';
            //遍历文章集合
            for (var i = 0; i < data.length; i++) {
                var item = data[i];
                html += "<tr>";
                html += "<td>" + item.link_id + "</td>";
                html += "<td><span class='update'>" + item.link_name + "</span><input class='dia' idd='"+item.link_id+"'  type='hidden'></td>";
                html += "<td><span class='upda'>" + item.link_url + "</span><input class='dian' idd='"+item.link_id+"'  type='hidden'</td>";
                html += '<td><button class="layui-btn layui-btn-small layui-btn-normal delete" idd="' + item.link_id + '"><i class="layui-icon">删除</i></button></td>';
                html += "</tr>";
            }
            html += '</tbody>';
            html += '</table>';
