...
layui.define(['laypage', 'layer', 'form', 'pagesize'], function (exports) {
    ...
    function initilData(currentIndex, pageSize,type="",textkey="",stime="",etime="") {
        var index = layer.load(1);
        //模拟数据
        var data = new Array();
        $.ajax({
              url:'http://blog.com/admin.php/Article/article_show.html',
              type:"get",
              data:"page="+currentIndex+"&size="+pageSize+"&type="+type+"&textkey="+textkey+"&stime="+stime+"&etime="+etime,
              dataType:'json',
        ...
        setTimeout(function () {
            ...
            html += '<thead><tr><th>标题</th><th>分类</th><th>图片</th><th>作者</th><th>添加时间</th><th colspan="2">操作</th></tr></thead>';
            html += '<tbody>';
            //遍历文章集合
            for (var i = 0; i < data.length; i++) {
                ...
                html += "<td>" + item.sort_name + "</td>";
                html += "<td><img src="+item.blog_img+" width='150px' hight='50px'></td>";
                html += "<td>" + item.blog_author + "</td>";
                html += "<td>" + formatDate(item.blog_time) + "</td>";
                html += '<td><a class="layui-btn layui-btn-small layui-btn-danger" href="http://blog.com/admin.php/Article/ar_update.html?id='+item.blog_id+'">修改</a></td>';
                html += '<td><button class="layui-btn layui-btn-small layui-btn-danger del"  idd="' + item.blog_id + '"><i class="layui-icon">&#xe640;</i></button></td>';
                html += "</tr>";
            }
            html += '</tbody>';
            html += '</table>';
            ...
            laypage({
                ...
                jump: function (obj, first) {
                    ...
                    if (!first) {
                        initilData(currentIndex, pageSize,type,textkey,stime,etime);
                    }
                }
            });
    ...
    //搜索
       $('#limit').click(function () {
        var index = layer.load(1);
        var type = $("#sleect option:selected").val();
        var textkey = $("#textkey").val();
        var stime = $('#stime').val();
        var etime = $('#etime').val();
        initilData(1,3,type=type,textkey=textkey,stime=stime,etime=etime);
        setTimeout(function () {
            layer.close(index);
            layer.msg('搜索成功 ');
        }, 500);
